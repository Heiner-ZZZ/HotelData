...
export function mapEditPropertySources(
  profileDto: PropertyProfileResponseDto,
  policiesDto: {
    policies?: {
  ...
  const profile = profileDto.profile;
  const hotel = profileDto.hotel;
  const policies = policiesDto?.policies ?? {};
  const amenities = amenitiesDto?.amenities ?? {};
  return {
    propId: profile.prop_id,
    hotelName: profile.hotel_name || hotel.hotel_name || hotel.display_name,
    displayName: profile.display_name || hotel.display_name,
    countryDisplayName: profile.display_country_label || hotel.country_display_name || '',
    originalGeneratedName: profile.original_generated_name || hotel.original_generated_name || hotel.display_name || `Hotel Partner ${profile.prop_id}`,
    manualOverride: profile.manual_override ?? hotel.manual_override ?? false,
    nameSource: profile.name_source ?? hotel.name_source ?? 'generated_from_id',
    profileBadge: profile.profile_badge || hotel.profile_badge || ((profile.manual_override ?? hotel.manual_override) ? 'Nombre editado manualmente' : 'Nombre generado'),
    updatedBy: profile.updated_by ?? '',
    updatedAt: profile.updated_at ?? '',
    ...
    policies: {
      ...
      petPolicy: policies.pet_policy ?? 'false',
      childrenPolicy: policies.children_policy ?? '',
      extraBedPolicy: policies.extra_bed_policy ?? '',
      paymentPolicy: policies.payment_policy ?? '',
      houseRules: policies.house_rules ?? '',
    },
    images: (amenitiesDto?.images ?? []).map((image) => ({
