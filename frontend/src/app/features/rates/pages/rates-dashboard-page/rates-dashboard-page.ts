...
export class RatesDashboardPageComponent {
  ...
  // ── Dashboard resource ──
  readonly dashboardResource = rxResource<RoomPerformanceDashboard, unknown>({
    params: () => {
    ...
    stream: ({ params }) => this.ratesApi.getRoomPerformanceDashboard({
  ...
  readonly roomTypeOptions = computed(() => this.data()?.summary.byRoomType ?? []);
  readonly channelOptions = computed(() => this.data()?.summary.byChannel ?? []);
  ...
  /** El gráfico central muestra solo el ADR (métrica R1.2); el revenue queda en los KPIs. */
  readonly chartDatasets = computed(() => {
    const ds = this.series().datasets;
    return ds.length ? [{ label: ds[0].label, data: ds[0].data }] : [];
  });
  ...
  toggleFilter(key: 'room_type_id' | 'channel', value: string, current: string): void {
  ...
  channelLabel(c: string): string {
    return ({ direct: 'Directo', direct_website: 'Web directa', booking_engine: 'Motor', ota: 'OTA', gds: 'GDS', walk_in: 'Walk-in', phone: 'Teléfono', corporate: 'Corporativo', agency: 'Agencia', tour_operator: 'Tour operador' })[c] ?? (c || 'Sin canal');
  }

  shortRoomType(roomTypeId: string): string {
    if (!roomTypeId) return '—';
    const prefix = 'RT-';
    if (roomTypeId.startsWith(prefix)) {
      return roomTypeId.slice(prefix.length).split('-').slice(1).join('-') || roomTypeId;
    }
    return roomTypeId;
  }
}
