...
@Component({
  ...
  template: `
    <section class="surface-card data-panel" style="margin-bottom: 1.25rem">
      <div class="panel-head collapsible-trigger" role="button" tabindex="0"
           [attr.aria-expanded]="!collapsed('roomTypes')"
           (click)="toggleCollapse('roomTypes')"
           (keydown.enter)="toggleCollapse('roomTypes'); $event.preventDefault()"
           (keydown.space)="toggleCollapse('roomTypes'); $event.preventDefault()">
        ...
        <span class="material-symbols-outlined collapse-icon">{{ collapsed('roomTypes') ? 'expand_more' : 'expand_less' }}</span>
      </div>
      @if (!collapsed('roomTypes')) {
    ...
    <section class="surface-card data-panel">
      <div class="panel-head collapsible-trigger" role="button" tabindex="0"
           [attr.aria-expanded]="!collapsed('hotelRooms')"
           (click)="toggleCollapse('hotelRooms')"
           (keydown.enter)="toggleCollapse('hotelRooms'); $event.preventDefault()"
           (keydown.space)="toggleCollapse('hotelRooms'); $event.preventDefault()">
        ...
        <span class="material-symbols-outlined collapse-icon">{{ collapsed('hotelRooms') ? 'expand_more' : 'expand_less' }}</span>
      </div>
      @if (!collapsed('hotelRooms')) {
...
export class RpTablesComponent {
  ...
  /** Secciones colapsadas. Por defecto solo la primera caja queda abierta. */
  private readonly collapsedSections = signal<Set<string>>(new Set(['hotelRooms']));
