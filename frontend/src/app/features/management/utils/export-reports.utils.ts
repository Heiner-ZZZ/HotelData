    ...
    sheets: [
      {
        ...
        rows: [
          ['Eventos totales', data.totalEvents] as any,
          ['Reservas detectadas', data.reservationsDetected] as any,
          ['Revenue bruto', data.grossRevenueLabel] as any,
          ['Colección fuente', data.sourceCollection] as any,
          ['Generado en', timestamp()] as any,
        ],
        column_widths: { A: 28, B: 30 },
      },
      {
        name: 'Top Hoteles',
        headers: [{ label: 'Hotel' }, { label: 'Perfil' }, { label: 'Eventos', align: 'right' } as any, { label: 'Revenue' }],
        rows: data.topHotels.map((h) => [h.displayName, h.profileBadge, h.events, h.grossRevenueLabel] as any),
        column_widths: { A: 32, B: 28, C: 14, D: 22 },
      },
      {
        name: 'Top Destinos',
        headers: [{ label: 'Destino' }, { label: 'Eventos', align: 'right' } as any, { label: 'Revenue' }],
        rows: data.topDestinations.map((d) => [d.label, d.events, d.grossRevenueLabel] as any),
        column_widths: { A: 28, B: 16, C: 22 },
      },
      {
        name: 'Top Países',
        headers: [{ label: 'País' }, { label: 'Eventos', align: 'right' } as any, { label: 'Reservas', align: 'right' } as any],
        rows: data.topVisitorCountries.map((c) => [c.label, c.events, c.reservations] as any),
        column_widths: { A: 28, B: 16, C: 16 },
      },
      {
        name: 'Colecciones',
        headers: [{ label: 'Colección' }, { label: 'Valor', align: 'right' } as any],
        rows: data.operationalCounts.map((o) => [o.label, Number(o.value) || o.value] as any),
