...
def _check_availability(
    ...
    # TODO: Ideally filter by rate_plans linked to room_type_id via rate_plans.room_type_id.
    #       Currently checks ALL open rate plans — conservative but may be too strict.
    ...
    if rate_records:
        ...
        if max_min_stay > requested_nights:
            return (
                f"Minimum stay is {max_min_stay} night(s). "
                f"Requested {requested_nights} night(s)."
            )
        ...
        if non_null_max_stay:
            ...
            if min_max_stay < requested_nights:
                return (
                    f"Maximum stay is {min_max_stay} night(s). "
                    f"Requested {requested_nights} night(s)."
                )
