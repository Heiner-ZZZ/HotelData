...
export class PropertyEditPageComponent {
    ...
    this.route.paramMap
      .pipe(
        ...
        switchMap((propId) => {
          ...
          return this.api.getEditProfile(propId);
        }),
  ...
  saveAll() {
    ...
    checks.push(this.api.savePolicies(propId, {
      ...
      check_out_time: currentVm.policies.checkOutTime,
    }).toPromise().then(() => true).catch(() => false));

    checks.push(this.api.saveAmenities(propId, currentVm.amenities).toPromise().then(() => true).catch(() => false));
  ...
  addImage() {
    ...
    this.api.addImage(this.propId(), url, title).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: () => {
  ...
  removeImage(imageUrl: string) {
    const current = this.vm();
    if (!current) return;
    this.api.deleteImage(this.propId(), imageUrl).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: () => {
