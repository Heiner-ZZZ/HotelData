...
import { CheckInsApiService } from '../../services/check-ins-api.service';
import { CheckInsSummaryCardsComponent } from '../../components/check-ins-summary-cards/check-ins-summary-cards';
import { CheckInsTableComponent } from '../../components/check-ins-table/check-ins-table';

function todayIso() { return new Date().toISOString().slice(0, 10); }

@Component({
  selector: 'app-check-ins-page',
  imports: [CheckInsSummaryCardsComponent, CheckInsTableComponent, EmptyStateComponent, ErrorStateComponent, LoadingStateComponent, PageHeaderComponent, ReactiveFormsModule],
...
export class CheckInsPageComponent {
  ...
  completeCheckIn(bookingId: string) {
    if (!confirm(`Confirmar check-in para ${bookingId}?`)) return;
    ...
    this.api.completeCheckIn(bookingId).pipe(
      ...
      error: (error: ApiError) => {
        this.errorMessage.set(error.message || 'No fue posible completar el check-in.');
        this.message.set('');
      }
    });
  }
