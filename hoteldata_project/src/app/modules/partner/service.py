...
def _rate_plans_for_prop(prop_id: int, limit: int = 50) -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        item["updated_at_label"] = _iso_label(item.get("updated_at"))
    return items
        ...
        enriched.append(
            {
                **item,
                ...
                "profile_badge": _profile_badge(item),
                "status": status,
        ...
        "top_hotels_by_revenue": [
            {
                ...
                "display_name": _hotel_display_name(hotel_lookup.get(int(item["_id"]), {}), int(item["_id"])),
        ...
        "top_destinations": [
            {
                ...
                "label": _destination_display_name(destination_lookup.get(int(item["_id"]), {}), int(item["_id"])),
                "events": int(item.get("events") or 0),
                "gross_revenue": round(float(item.get("gross_revenue") or 0), 2),
            }
...
def create_rate_plan(
    ...
    *,
    ...
    rate_plan_id = f"RP-{prop_id}-{_slugify(clean_name)}"
    try:
...
def save_rate_calendar_entry(
    ...
    *,
    ...
    min_stay_value = max(_safe_positive_int(min_stay_nights, 1), 1)
    payload = {
        "prop_id": prop_id,
